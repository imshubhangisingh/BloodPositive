...
app.config['MAIL_USERNAME'] = 'emailid@domainname.com'
app.config['MAIL_PASSWORD'] =  'emailpassword'
mail=Mail(app)
s=URLSafeTimedSerializer(app.secret_key)

#Config MySQL on local
...
#Config MySQL on Heroku
app.config['MYSQL_DATABASE_USER'] = 'username'
app.config['MYSQL_DATABASE_PASSWORD'] = 'userpassword'
app.config['MYSQL_DATABASE_DB'] = 'heroku_dbname'
        ...
        if result > 0:
            ...
            for i in search:
                ...
                if(i['PHONE_NUMBER']):
                    ...
                    body+='\n'+"Thank You!!"
     #add a sender email
            cur.execute("INSERT INTO request(CITY,BLOOD_GROUP,verify,username)VALUES(%s, %s, %s,%s)",(CITY,BLOOD_GROUP,0,session['USERNAME']))
            ...
            l="The donors have been notified of your request."
...
def dashboard():
    ...
    cur =mysql.connection.cursor()
    #Get Aricle
    result = cur.execute("SELECT * FROM request where username=%s",[session['USERNAME']])
    articles = cur.fetchall()
    if result>0:
...
def username():
    if request.method == 'POST':
        ...
        if USERNAME != session['USERNAME']:
            msg="You can only edit your Username."
...
def show():
    cur =mysql.connection.cursor()
    #Get Aricle
    result = cur.execute("SELECT * FROM request WHERE verify=1 and username=%s",[session['USERNAME']])
    fulfill = cur.fetchall()
    if result>0:
        return render_template('show.html', fulfill=fulfill)
    else:
        msg= 'No Fulfilled requests found'
        return render_template('dashboard.html',error =msg)
    #close conection
